...
class RpcServer(object):
    def run_fifo(self, data_pn, signal_pn):
        while True:
            with open(signal_pn, 'r') as sf:
                with open(data_pn, 'r') as df:
                    req = df.read()
            try:
                ...
                with open(data_pn, 'w') as df:
                    df.write(resp)
                with open(signal_pn, 'w') as f:
                    pass
            except:
                ## Open and close the FIFO to wake up the client
                with open(data_pn, 'w') as df:
                    pass
                with open(signal_pn, 'w') as sf:
                    pass
                raise
