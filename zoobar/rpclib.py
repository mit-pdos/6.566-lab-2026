...
class RpcClientFifo(object):
    def __init__(self, data_pn, signal_pn):
        self.data_pn = data_pn
        self.signal_pn = signal_pn

    def call(self, method, **kwargs):
        req = format_req(method, kwargs)
        with open(self.data_pn, 'w') as df:
            df.write(req)
        with open(self.signal_pn, 'w') as sf:
            pass
        with open(self.signal_pn, 'r') as sf:
            with open(self.data_pn, 'r') as df:
                resp = df.read()
