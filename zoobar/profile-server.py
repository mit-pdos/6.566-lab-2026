...
class FifoServer(object):
    def __init__(self, server, data_pn, fifo_pn):
        with open(data_pn, 'w') as f:
            pass
        os.mkfifo(fifo_pn)
        self.server = server
    ...
    def __enter__(self):
        self.pid = os.fork()
        if self.pid == 0:
            self.server.run_fifo(self.data_pn, self.fifo_pn)
            os._exit(0)
...
class ProfileServer(rpcsrv.RpcServer):
    def rpc_run(self, pcode, user, visitor):
        ...
        ## through two special files, td/rpc.data and td/rpc.fifo.
        with tempfile.TemporaryDirectory() as td, FifoServer(ProfileAPIServer(user, visitor, pcode), td + "/rpc.data", td + "/rpc.fifo") as ps:
            ...
            ## to communicate with the ProfileAPIServer through two special
            ## files at /run/rpc.data and /run/rpc.fifo.  You will need to
            ## arrange for this to work.
