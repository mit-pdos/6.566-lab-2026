...
export var webpage = (() => {
  return {
    create: async () => {
      ...
      // forward page errors and such to grader output for easier debugging
      page.on('pageerror', ({ message }) => console.log('Page error:', message));
      page.on('console', message => console.log(`Page console: ${message.type().substr(0, 3).toUpperCase()} ${message.text()}`));
...
async function zoobarLoginHelper(origin, submit, user, cred, loginOpts, cb) {
    ...
    await openOrDie(page, base + "login", async function() {
        ...
        await page.evaluate(function(user, overrideCredsGetChallenge, corruptSignature) {
            ...
            navigator.credentials.get = async function (options) {
                ...
                if (corruptSignature && res && res.response && res.response.signature) {
                    var view = new Uint8Array(res.response.signature);
        ...
        }, user, loginOpts.overrideCredsGetChallenge, loginOpts.corruptSignature);
