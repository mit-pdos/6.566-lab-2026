...
def dbquery(container, dbfile, q):
    ## sqlite3 is very particular about canonicalizing pathnames,
    ## which doesn't work so well with container namespaces..
    dbpath = container_path(container, dbfile)
    shutil.copyfile(dbpath, "/tmp/grader.db")
    conn = sqlite3.connect("/tmp/grader.db")
    cur  = conn.cursor()
    cur.execute(q)
    ret  = cur.fetchall()
    cur.close()
